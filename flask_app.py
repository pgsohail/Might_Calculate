...
class Variables:
    def __init__(self):
        ...
        self.EXPIRATION_DATE =   ''                           # 20231019_18  代表 2023/10/19 18:00:00 到期
        self.PRICE_LEVEL =       ''
...
def certificate_key_GTA_Auto_Play():
    ...
    if 'new_certificate_key' in my_dict.keys():
        new_certificate_key = my_dict['new_certificate_key']
    else:
        new_certificate_key = 'None'
    ...
    # print(f'user: {user}, status: {status}, version: {version}, new_certificate_key: {new_certificate_key}')
    print(f'status: {status:14}, ver: {version:12}, user: {user}')
...
def certificate_key_animal_restaurant():
    ...
    # print(f'user: {user}, status: {status}, ver: {version}, new_certificate_key: {new_certificate_key}')
    print(f'status: {status:14}, ver: {version:12}, user: {user}')
    ...
    for line in lines:
        ...
        if pure_line.find(user) != -1:
            EXPIRATION_DATE = pure_line.split(',')[1]
            PRICE_LEVEL = pure_line.split(',')[2]
            FLAG = f'{my_var.PASS_MSG},{EXPIRATION_DATE},{PRICE_LEVEL}'
            break
...
@app.route('/certificate_key_HarryPotter_Magic_Awakened', methods=['POST'])
def certificate_key_HarryPotter_Magic_Awakened():
    if request.is_json:
        json_str = request.get_json()
    else:
        abort(404)
    ...
    login_file = 'login_HarryPotter_Magic_Awakened.txt'
    mac_file = 'mac_HarryPotter_Magic_Awakened.txt'
    new_user_file = 'new_user_HarryPotter_Magic_Awakened.txt'
    valid_version_list = version_dict['HarryPotter_Magic_Awakened']['VALID_VERSION']
